...
def create_testbed_file(pargs, hosts, openrc_dict):
    ...
    env_password = {}
    env_hostnames = {}
    host_build = {"build": ['host_build']}
    login_name = 'heat-admin'
    is_analytics_isolated = False
    env_password.update({'host_build': 'c0ntrail123'})
    env_roledefs.update(host_build)
    for host in hosts:
        ...
        env_hostnames.update({host_name : host['host_name']})
    ...
    env_hostnames.update({'host_build' : 'unedrcloud'})
    env_roledefs.update(host_build)
...
#    tb_list.append("env.test = %s"%json.dumps(env_test, sort_keys=True, indent=4))
    ...
    tb_list.append("env.hostnames = %s"%json.dumps(env_hostnames, sort_keys=True, indent=4))
    tb_list.append("env.passwords = %s"%json.dumps(env_password, sort_keys=True, indent=4))
 
    # RHOSP sanity do not need control_date. Remove below comment for generating control_data.
...
def parse_cli(args):
    ...
    parser.add_argument('--tb_filename', default='testbed-new.py', help='Testbed output file name')
